...
namespace fac.ASTs.Exprs {
	public class AstExpr_Array: IAstExpr {
		...
		public override IAstExpr TraversalCalcType (IAstType? _expect_type) {
			...
			// 处理类型
			bool _success = InitCount.TraversalCalcTypeWrap (IAstType.FromName ("int"), a => InitCount = a);
			for (int i = 0; i < InitValues.Count; ++i) {
				_success &= InitValues[i].TraversalCalcTypeWrap (ItemDataType, a => InitValues[i] = a);
			}
			if (!_success)
				return null;
		...
		public override (List<IAstStmt>, IAstExpr) ExpandExpr ((IAstExprName _var, AstStmt_Label _pos)? _cache_err) {
			...
			var _defvar_stmt = new AstStmt_DefVariable { Token = Token, DataType = ExpectType, VarName = _var_id, Expr = AstExpr_ArrayAPI.Array_New (ExpectType) };

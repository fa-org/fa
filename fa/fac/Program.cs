...
namespace fac {
    class Program {
        ...
        private static async Task CheckUpdates(Version version) {
            ...
            try {
                var streamTask = client.GetStreamAsync("https://api.github.com/repos/fa-org/fa/releases");
                var repositories = await JsonSerializer.DeserializeAsync<List<Releases>>(await streamTask);
        ...
        static async Task Main(string[] args) {
            ...
            await CheckUpdates(_asm.GetName().Version);
            Console.WriteLine();
            // 检查更新
